    ...
    fetch('codedatas.php')
        ...
        .then(data => {
            ...
            data.data.forEach(item => {
                // 跳过 null 或 undefined 项
                if (!item) return;

                const code = item.code || '无数据';
                const time = item.time ? Math.floor(item.time / 1440) + ' 天' : '无数据';
                const activation = item.activation ? '已激活' : '未激活';

                // 将 starttime 和 entime 为 0 的情况显示为“未设置”
                const starttime = item.starttime && item.starttime !== 0 
                    ? new Date(item.starttime * 1000).toLocaleDateString() 
                    : '未设置';
                const entime = item.entime && item.entime !== 0 
                    ? new Date(item.entime * 1000).toLocaleDateString() 
                    : '未设置';

                const row = tableBody.insertRow();
                row.innerHTML = `
                    <td><button class="delete-btn" onclick="deleteCode('${code}')">删除</button></td>
                    <td>${code}</td>
                    <td>${time}</td>
                    <td>${activation}</td>
                    <td>${starttime}</td>
